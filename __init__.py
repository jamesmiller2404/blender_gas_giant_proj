"""
Gas Giant Shader Controls v8 - fixed six-layer chaining, diagnostics, atmosphere, and direct swirl controls
...
bl_info = {
    "name": "Gas Giant Shader Controls v8",
    "author": "ChatGPT for James Miller",
    "version": (8, 0, 0),
    "blender": (4, 0, 0),
    "location": "View3D > Sidebar > Gas Giant",
    "description": "Safer 6-layer gas giant shader controller with presets, atmospheric limb nodes, and direct swirl structure controls plus repaired six-layer chaining.",
    "category": "Material",
}
...
SWIRL_FRAME_NAME = "GG Cloud Swirl Structure Rig"
...
def setup_controller(ctrl):
    ...
    defaults = {
        ...
        'swirl_tightness': 7.0,
        'swirl_curvature': 0.65,
        'swirl_scale': 1.0,
        'swirl_offset': 0.0,
        'swirl_layer_variation': 0.45,
    }
    ...
    for prop_name, min_value, max_value in [
        ('swirl_tightness', 0.1, 40.0),
        ('swirl_curvature', 0.0, 25.0),
        ('swirl_scale', 0.0, 2.0),
...
def setup_controller_after_register():
    ...
    try:
        ...
        print(f"Gas Giant Shader Controls: controller setup deferred/failed: {exc}")
    return None
...
def build_rig(context):
    ...
    for i in range(1, 7):
        ...
        math = get_or_new(nt, 'ShaderNodeMath', f'GG Layer Strength Math {i}', (-900, y))
    ...
    mat['gas_giant_v8_note'] = 'Built by Gas Giant Shader Controls v8. Six cloud layers are serially chained and independently driven by controller properties.'
    return mat.name
...
def build_swirl_warp(context):
    """Add/repair direct cloud-structure swirl nodes.

    v6 tried to warp noise coordinates. In many Blender node setups that can look like it
    does nothing because the existing layer masks dominate the result. v7 is more direct:
    it inserts a Wave Texture / Noise Texture / ColorRamp mask into each cloud layer's
    factor path, between GG Mask Ramp i and GG Layer Strength Math i. That means the swirl
    controls visibly alter the cloud structures instead of only changing invisible vectors.

    Controls:
    - swirl_tightness: Wave Texture scale; higher = tighter/smaller swirls/bands.
    - swirl_curvature: Wave Texture distortion; higher = more bending/turbulence.
    - swirl_scale: influence amount; 0 = original clouds, 1 = strong swirl modulation.
    - swirl_offset: phase offset; can be keyframed to animate drift.
    - swirl_layer_variation: offsets each layer so they do not all have the same swirl.
    """
    ctrl = find_controller()
    setup_controller(ctrl)
    mat = find_material(context)
    ...
    if not nodes.get('GG Noise 1') or not nodes.get('GG Mask Ramp 1'):
        build_rig(context)
        nt = mat.node_tree
        nodes = nt.nodes
    ...
    frame.location = (-1780, -840)

    texcoord = get_or_new(nt, 'ShaderNodeTexCoord', 'GG Swirl Structure Coordinates', (-1780, -520))
    mapping = get_or_new(nt, 'ShaderNodeMapping', 'GG Swirl Structure Mapping', (-1540, -520))
    ...
    if rot_socket:
        drive_prop_index(nt, f'nodes["{mapping.name}"].inputs["{rot_socket.name}"].default_value', 2, ctrl, 'swirl_layer_variation', 'var*0.35')

    wired = 0
    for i in range(1, 7):
        mask_ramp = nodes.get(f'GG Mask Ramp {i}')
        layer_math = nodes.get(f'GG Layer Strength Math {i}') or nodes.get(f'GG Strength {i}')
        if not mask_ramp or not layer_math:
            continue

        y = -620 - i * 150
        wave = get_or_new(nt, 'ShaderNodeTexWave', f'GG Swirl Structure Wave {i}', (-1300, y))
        noise = get_or_new(nt, 'ShaderNodeTexNoise', f'GG Swirl Structure Noise {i}', (-1300, y - 115))
        addmask = get_or_new(nt, 'ShaderNodeMath', f'GG Swirl Structure Add {i}', (-1040, y))
        swirl_ramp = get_or_new(nt, 'ShaderNodeValToRGB', f'GG Swirl Structure Ramp {i}', (-820, y))
        influence = get_or_new(nt, 'ShaderNodeMix', f'GG Swirl Structure Influence {i}', (-580, y))
        multiply = get_or_new(nt, 'ShaderNodeMath', f'GG Swirl Structure Apply {i}', (-350, y))

        for node in [wave, noise, addmask, swirl_ramp, influence, multiply]:
            node.parent = frame

        # Configure node modes defensively. Blender 4.x supports ShaderNodeMix with data_type.
        try:
        ...
        try:
            ...
            influence.factor_mode = 'UNIFORM'
            influence.clamp_factor = True
        except Exception:
            pass

        # Wave controls: tightness and curvature should be very obvious.
        if 'Vector' in wave.inputs:
            clear_input(nt, wave.inputs['Vector']); link(nt, mapping.outputs[0], wave.inputs['Vector'])
        if 'Scale' in wave.inputs:
            # Per-layer variation changes apparent tightness per layer.
            expr = f'var*{1.0 + i*0.08:.4f}'
            drive_prop(nt, f'nodes["{wave.name}"].inputs["Scale"].default_value', ctrl, 'swirl_tightness', expr)
        if 'Distortion' in wave.inputs:
            drive_prop(nt, f'nodes["{wave.name}"].inputs["Distortion"].default_value', ctrl, 'swirl_curvature', 'var')
        if 'Phase Offset' in wave.inputs:
            drive_prop(nt, f'nodes["{wave.name}"].inputs["Phase Offset"].default_value', ctrl, 'swirl_offset', f'var+{i*0.11:.4f}')

        # Noise adds turbulent breakup to the wave so it looks less like perfect stripes.
        if 'Vector' in noise.inputs:
            clear_input(nt, noise.inputs['Vector']); link(nt, mapping.outputs[0], noise.inputs['Vector'])
        if 'Scale' in noise.inputs:
            drive_prop(nt, f'nodes["{noise.name}"].inputs["Scale"].default_value', ctrl, 'swirl_tightness', f'var*{0.45 + i*0.06:.4f}')
        if 'Detail' in noise.inputs:
            drive_prop(nt, f'nodes["{noise.name}"].inputs["Detail"].default_value', ctrl, 'cloud_complexity', 'var')
        if 'Roughness' in noise.inputs:
            drive_prop(nt, f'nodes["{noise.name}"].inputs["Roughness"].default_value', ctrl, 'cloud_roughness', 'var')

        # Combine wave and noise before thresholding.
        clear_input(nt, addmask.inputs[0]); link(nt, socket_by_names(wave.outputs, ['Color','Fac'], 1), addmask.inputs[0])
        clear_input(nt, addmask.inputs[1]); link(nt, socket_by_names(noise.outputs, ['Fac','Color'], 0), addmask.inputs[1])

        # Swirl mask ramp. This controls the tight visual edge of swirls.
        while len(swirl_ramp.color_ramp.elements) < 2:
            swirl_ramp.color_ramp.elements.new(1.0)
        while len(swirl_ramp.color_ramp.elements) > 2:
            swirl_ramp.color_ramp.elements.remove(swirl_ramp.color_ramp.elements[-1])
        swirl_ramp.color_ramp.interpolation = 'EASE'
        swirl_ramp.color_ramp.elements[0].color = (0, 0, 0, 1)
        swirl_ramp.color_ramp.elements[1].color = (1, 1, 1, 1)
        swirl_ramp.color_ramp.elements[0].position = 0.30
        swirl_ramp.color_ramp.elements[1].position = 0.70

        clear_input(nt, swirl_ramp.inputs[0]); link(nt, addmask.outputs[0], swirl_ramp.inputs[0])

        # Mix factor: swirl_scale is now influence amount. 0 keeps a neutral mask of 1.0;
        # 1 uses the generated swirl mask fully. This is intentionally direct and visible.
        fac = socket_by_names(influence.inputs, ['Factor', 'Fac'], 0)
        a = socket_by_names(influence.inputs, ['A'], 6) or (influence.inputs[6] if len(influence.inputs) > 6 else None)
        b = socket_by_names(influence.inputs, ['B'], 7) or (influence.inputs[7] if len(influence.inputs) > 7 else None)
        if fac:
            drive_prop(nt, f'nodes["{influence.name}"].inputs["{fac.name}"].default_value', ctrl, 'swirl_scale', 'var')
        if a:
            try:
                a.default_value = 1.0
            except Exception:
                pass
        if b:
            clear_input(nt, b); link(nt, swirl_ramp.outputs[0], b)

        # If ShaderNodeMix socket lookup failed in this Blender build, fall back to using
        # the ramp directly; the control will still be visible, just less bypassable.
        influence_out = influence.outputs[0] if len(influence.outputs) else None
        if not influence_out:
            influence_out = swirl_ramp.outputs[0]

        # Insert between the original GG Mask Ramp and the layer strength multiply node.
        # Result = original mask * swirl influence mask.
        clear_input(nt, multiply.inputs[0]); link(nt, mask_ramp.outputs[0], multiply.inputs[0])
        clear_input(nt, multiply.inputs[1]); link(nt, influence_out, multiply.inputs[1])
        clear_input(nt, layer_math.inputs[0]); link(nt, multiply.outputs[0], layer_math.inputs[0])
        wired += 1

    mat['gas_giant_swirl_note'] = f'Direct cloud-structure swirl rig added by Gas Giant Shader Controls v8; wired {wired} layer masks.'
    return mat.name, wired

def apply_test_swirl(ctrl):
    setup_controller(ctrl)
    # Very strong settings so the effect is unmistakable.
    ctrl['swirl_tightness'] = 12.0
    ctrl['swirl_curvature'] = 8.0
    ctrl['swirl_scale'] = 1.0
...
def repair_layer_chain(context):
    """Force the six GG cloud layers into a simple serial MixRGB chain.

    This is the important v8 fix. Earlier versions could leave links from old rigs,
    or the swirl routine could look for a different strength-node name. This routine
    deliberately reconnects every layer so each one contributes to the final Base Color:

    base -> Mix 1 -> Mix 2 -> Mix 3 -> Mix 4 -> Mix 5 -> Mix 6 -> Hue/Sat -> Principled Base Color
    """
    ctrl = find_controller()
    setup_controller(ctrl)
    mat = find_material(context)
    ...
    # Make sure the rig exists first.
    if not nodes.get('GG Layer Mix 1') or not nodes.get('GG Layer Strength Math 1'):
        build_rig(context)
        nt = mat.node_tree
        nodes = nt.nodes

    base_rgb = nodes.get('GG Fallback Base Color')
    previous = base_rgb.outputs[0] if base_rgb else None
    if previous is None:
        raise RuntimeError('Could not find GG Fallback Base Color output.')

    repaired = 0
    for i in range(1, 7):
        # Accept old strength-node names but normalize by using/creating the new one.
        old_strength = nodes.get(f'GG Strength {i}')
        strength = nodes.get(f'GG Layer Strength Math {i}') or old_strength
        ramp = nodes.get(f'GG Mask Ramp {i}')
        color = nodes.get(f'GG Layer Color {i}')
        mix = nodes.get(f'GG Layer Mix {i}')
        if not all([strength, ramp, color, mix]):
            continue

        # Ensure the layer strength node is correctly driven and fed from its own mask.
        try:
            strength.operation = 'MULTIPLY'
        except Exception:
            pass
        if len(strength.inputs) >= 2:
            if not strength.inputs[0].is_linked:
                clear_input(nt, strength.inputs[0]); link(nt, ramp.outputs[0], strength.inputs[0])
            drive_prop(nt, f'nodes["{strength.name}"].inputs[1].default_value', ctrl, f'layer_{i}_strength', 'var')

        fac, a, b, out = mix_sockets(mix)
        if not all([fac, a, b, out]):
            continue
        clear_input(nt, fac); clear_input(nt, a); clear_input(nt, b)
    ...
    hue = nodes.get('GG Hue Saturation Value')
    if hue:
        color_socket = socket_by_names(hue.inputs, ['Color'], len(hue.inputs)-1)
        color_output = socket_by_names(hue.outputs, ['Color'], 0)
        clear_input(nt, color_socket)
        ...
        clear_input(nt, base_input)
        link(nt, previous, base_input)

    mat['gas_giant_v8_layer_chain_note'] = f'v8 repaired serial cloud chain; {repaired} layer mixes connected.'
    return mat.name, repaired


def set_layer_test(ctrl, layer_number):
    setup_controller(ctrl)
    for i in range(1, 7):
        ctrl[f'layer_{i}_strength'] = 1.0 if i == layer_number else 0.0


class GASGIANT_OT_repair_layers_v8(bpy.types.Operator):
    bl_idname = 'gasgiant.repair_layers_v8'
    bl_label = 'Repair / Force All 6 Layers'
    bl_description = 'Force all six cloud layers into one serial chain so each layer control affects the visible planet'
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        try:
            mat_name, repaired = repair_layer_chain(context)
        except Exception as e:
            self.report({'ERROR'}, str(e))
            print('Gas Giant v8 layer repair error:', repr(e))
            return {'CANCELLED'}
        self.report({'INFO'}, f'Repaired {repaired}/6 cloud layers on {mat_name}.')
        return {'FINISHED'}


class GASGIANT_OT_test_single_layer_v8(bpy.types.Operator):
    bl_idname = 'gasgiant.test_single_layer_v8'
    bl_label = 'Solo Selected Layer'
    bl_description = 'Set the selected cloud layer to 1.0 and all other cloud layers to 0.0 for testing'
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        ctrl = find_controller()
        layer = int(context.scene.gas_giant_test_layer)
        set_layer_test(ctrl, layer)
        self.report({'INFO'}, f'Solo test: Layer {layer} = 1.0; all other layers = 0.0')
        return {'FINISHED'}
...
class GASGIANT_OT_build_swirl(bpy.types.Operator):
    ...
    bl_label = 'Build / Repair Direct Swirl Structure'
    bl_description = 'Insert visible wave/noise swirl masks into the six cloud layer factors'
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        try:
            ...
            return {'CANCELLED'}
        self.report({'INFO'}, f'Built direct swirl structure on {mat_name}; wired {wired} layer masks.')
        return {'FINISHED'}
...
class GASGIANT_OT_test_swirl(bpy.types.Operator):
    ...
    bl_description = 'Set exaggerated swirl values so the direct structure effect is easy to confirm'
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        ctrl = find_controller()
        apply_test_swirl(ctrl)
        self.report({'INFO'}, 'Swirl test values applied: high direct swirl modulation.')
        return {'FINISHED'}
...
class GASGIANT_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator('gasgiant.repair_layers_v8', icon='MODIFIER')
        ...
        box = layout.box(); box.label(text='Direct Cloud Swirl / Curvature')
...
CLASSES = [
    ...
    GASGIANT_OT_repair_layers_v8,
    GASGIANT_OT_test_single_layer_v8,
    GASGIANT_OT_build,
...
def register():
    ...
    if not bpy.app.timers.is_registered(setup_controller_after_register):
        bpy.app.timers.register(setup_controller_after_register, first_interval=0.1)


def unregister():
    if bpy.app.timers.is_registered(setup_controller_after_register):
        bpy.app.timers.unregister(setup_controller_after_register)
...
if __name__ == '__main__':
    register()
    print('Gas Giant Shader Controls v7 registered. Press N > Gas Giant > Diagnostics, then Build / Repair 6-Layer Rig, Build / Repair Atmosphere Edge, or Build / Repair Direct Swirl Structure.')
